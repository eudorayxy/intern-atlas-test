...
def get_samples_magic(skim, string_code_dict):
    ...
    filepath_dict = validate_files(samples, skim)
...
def analysis_uproot(skim, string_code_dict, luminosity, fraction, read_variables, save_variables, cut_function,
                     write_txt=False, txt_filename=None,
                    write_parquet=False, output_directory=None, return_output=True):
    
    time_start = time.time()

    samples = get_samples_magic(skim, string_code_dict)
